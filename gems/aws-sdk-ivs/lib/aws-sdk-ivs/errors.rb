...
module AWS::SDK::Ivs
  module Errors
    ...
    class ChannelNotBroadcasting < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ChannelNotBroadcasting.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ChannelNotBroadcasting]
      #
      attr_reader :data
    end
    ...
    class PendingVerification < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::PendingVerification.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::PendingVerification]
      #
      attr_reader :data
    end
    ...
    class StreamUnavailable < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::StreamUnavailable.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::StreamUnavailable]
      #
      attr_reader :data
    end
