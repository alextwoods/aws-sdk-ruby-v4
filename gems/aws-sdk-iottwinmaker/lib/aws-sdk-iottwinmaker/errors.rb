...
module AWS::SDK::IoTTwinMaker
  module Errors
    ...
    class ConnectorFailureException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConnectorFailureException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConnectorFailureException]
      #
      attr_reader :data
    end

    class ConnectorTimeoutException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConnectorTimeoutException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConnectorTimeoutException]
      #
      attr_reader :data
    end
