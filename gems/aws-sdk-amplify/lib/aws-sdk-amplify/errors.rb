...
module AWS::SDK::Amplify
  module Errors
    ...
    class DependentServiceFailureException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DependentServiceFailureException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DependentServiceFailureException]
      #
      attr_reader :data
    end
