...
module AWS::SDK::GreengrassV2
  module Errors
    ...
    class RequestAlreadyInProgressException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::RequestAlreadyInProgressException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::RequestAlreadyInProgressException]
      #
      attr_reader :data
    end
