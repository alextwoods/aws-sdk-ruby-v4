  ...
  module Parsers
    ...
    class AcceptAddressTransfer
      def self.parse(http_resp)
        ...
        xml.at('addressTransfer') do |node|
          data.address_transfer = AddressTransfer.parse(node)
        end
        data
      end
    end
    ...
    class AddedPrincipal
      def self.parse(xml)
        ...
        xml.at('servicePermissionId') do |node|
          data.service_permission_id = (node.text || '')
        end
    ...
    class AddedPrincipalSet
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << AddedPrincipal.parse(node)
        end
        data
      end
    end
    ...
    class AdditionalDetail
      def self.parse(xml)
        ...
        xml.at('vpcEndpointService') do |node|
          data.vpc_endpoint_service = AnalysisComponent.parse(node)
        end
        xml.at('ruleOptionSet') do |node|
          children = node.children('item')
          data.rule_options = RuleOptionList.parse(children)
        end
        xml.at('ruleGroupTypePairSet') do |node|
          children = node.children('item')
          data.rule_group_type_pairs = RuleGroupTypePairList.parse(children)
        end
        xml.at('ruleGroupRuleOptionsPairSet') do |node|
          children = node.children('item')
          data.rule_group_rule_options_pairs = RuleGroupRuleOptionsPairList.parse(children)
        end
        xml.at('serviceName') do |node|
          data.service_name = (node.text || '')
        end
        xml.at('loadBalancerSet') do |node|
          children = node.children('item')
          data.load_balancers = AnalysisComponentList.parse(children)
        end
        return data
      end
    end
    ...
    class AddressTransfer
      def self.parse(xml)
        data = Types::AddressTransfer.new
        xml.at('publicIp') do |node|
          data.public_ip = (node.text || '')
        end
        xml.at('allocationId') do |node|
        ...
        xml.at('transferAccountId') do |node|
          data.transfer_account_id = (node.text || '')
        end
        xml.at('transferOfferExpirationTimestamp') do |node|
          data.transfer_offer_expiration_timestamp = Time.parse(node.text) if node.text
        end
        xml.at('transferOfferAcceptedTimestamp') do |node|
          data.transfer_offer_accepted_timestamp = Time.parse(node.text) if node.text
        end
        xml.at('addressTransferStatus') do |node|
          data.address_transfer_status = (node.text || '')
        end
        return data
      end
    end
    ...
    class AddressTransferList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << AddressTransfer.parse(node)
        end
        data
      end
    end
    ...
    class AssociateIpamResourceDiscovery
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('ipamResourceDiscoveryAssociation') do |node|
          data.ipam_resource_discovery_association = IpamResourceDiscoveryAssociation.parse(node)
        end
        data
      end
    end
    ...
    class AssociateTransitGatewayPolicyTable
      def self.parse(http_resp)
        ...
        xml.at('association') do |node|
          data.association = TransitGatewayPolicyTableAssociation.parse(node)
        end
        data
      end
    end
    ...
    class AttachVerifiedAccessTrustProvider
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessTrustProvider') do |node|
          data.verified_access_trust_provider = VerifiedAccessTrustProvider.parse(node)
        end
        xml.at('verifiedAccessInstance') do |node|
          data.verified_access_instance = VerifiedAccessInstance.parse(node)
        end
        data
      end
    end
    ...
    class AttachmentEnaSrdSpecification
      def self.parse(xml)
        data = Types::AttachmentEnaSrdSpecification.new
        xml.at('enaSrdEnabled') do |node|
          data.ena_srd_enabled = (node.text == 'true')
        end
        xml.at('enaSrdUdpSpecification') do |node|
          data.ena_srd_udp_specification = AttachmentEnaSrdUdpSpecification.parse(node)
        end
        return data
      end
    end
    ...
    class AttachmentEnaSrdUdpSpecification
      def self.parse(xml)
        data = Types::AttachmentEnaSrdUdpSpecification.new
    ...
    class CapacityAllocation
      def self.parse(xml)
        data = Types::CapacityAllocation.new
        xml.at('allocationType') do |node|
    ...
    class CapacityAllocations
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << CapacityAllocation.parse(node)
        end
        data
      end
    end
    ...
    class CloudWatchLogOptions
      def self.parse(xml)
        data = Types::CloudWatchLogOptions.new
        xml.at('logEnabled') do |node|
          data.log_enabled = (node.text == 'true')
        end
        xml.at('logGroupArn') do |node|
          data.log_group_arn = (node.text || '')
        end
        xml.at('logOutputFormat') do |node|
          data.log_output_format = (node.text || '')
        end
        return data
      end
    end
    ...
    class CpuOptions
      def self.parse(xml)
        ...
        xml.at('amdSevSnp') do |node|
          data.amd_sev_snp = (node.text || '')
        end
        return data
      end
    end
    ...
    class CreateCoipCidr
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('coipCidr') do |node|
          data.coip_cidr = CoipCidr.parse(node)
        end
        data
      end
    end
    ...
    class CreateCoipPool
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('coipPool') do |node|
          data.coip_pool = CoipPool.parse(node)
        end
        data
      end
    end
    ...
    class CreateInstanceConnectEndpoint
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('instanceConnectEndpoint') do |node|
          data.instance_connect_endpoint = Ec2InstanceConnectEndpoint.parse(node)
        end
        xml.at('clientToken') do |node|
          data.client_token = (node.text || '')
        end
    ...
    class CreateIpamResourceDiscovery
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('ipamResourceDiscovery') do |node|
          data.ipam_resource_discovery = IpamResourceDiscovery.parse(node)
        end
        data
      end
    end
    ...
    class CreateLocalGatewayRouteTable
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('localGatewayRouteTable') do |node|
          data.local_gateway_route_table = LocalGatewayRouteTable.parse(node)
        end
        data
      end
    end
    ...
    class CreateLocalGatewayRouteTableVirtualInterfaceGroupAssociation
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('localGatewayRouteTableVirtualInterfaceGroupAssociation') do |node|
          data.local_gateway_route_table_virtual_interface_group_association = LocalGatewayRouteTableVirtualInterfaceGroupAssociation.parse(node)
        end
        data
      end
    end
    ...
    class CreateSnapshot
      def self.parse(http_resp)
        ...
        xml.at('sseType') do |node|
          data.sse_type = (node.text || '')
        end
        data
      end
    end
    ...
    class CreateTransitGatewayPolicyTable
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('transitGatewayPolicyTable') do |node|
          data.transit_gateway_policy_table = TransitGatewayPolicyTable.parse(node)
        end
        data
      end
    end
    ...
    class CreateTransitGatewayRouteTableAnnouncement
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('transitGatewayRouteTableAnnouncement') do |node|
          data.transit_gateway_route_table_announcement = TransitGatewayRouteTableAnnouncement.parse(node)
        end
        data
      end
    end
    ...
    class CreateVerifiedAccessEndpoint
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessEndpoint') do |node|
          data.verified_access_endpoint = VerifiedAccessEndpoint.parse(node)
        end
        data
      end
    end
    ...
    class CreateVerifiedAccessGroup
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessGroup') do |node|
          data.verified_access_group = VerifiedAccessGroup.parse(node)
        end
        data
      end
    end
    ...
    class DataResponse
      def self.parse(xml)
        ...
        xml.at('metricPointSet') do |node|
          children = node.children('item')
          data.metric_points = MetricPoints.parse(children)
        end
        return data
      end
    end
    ...
    class DescribeAddressTransfers
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('addressTransferSet') do |node|
          children = node.children('item')
          data.address_transfers = AddressTransferList.parse(children)
        end
    ...
    class DescribeAwsNetworkPerformanceMetricSubscriptions
      def self.parse(http_resp)
        ...
        xml.at('subscriptionSet') do |node|
          children = node.children('item')
          data.subscriptions = SubscriptionList.parse(children)
        end
        data
      end
    end
    ...
    class DescribeImageAttribute
      def self.parse(http_resp)
        ...
        xml.at('imdsSupport') do |node|
          data.imds_support = AttributeValue.parse(node)
        end
        data
      end
    end
    ...
    class DescribeInstanceConnectEndpoints
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('instanceConnectEndpointSet') do |node|
          children = node.children('item')
          data.instance_connect_endpoints = InstanceConnectEndpointSet.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeIpamResourceDiscoveries
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('ipamResourceDiscoverySet') do |node|
          children = node.children('item')
          data.ipam_resource_discoveries = IpamResourceDiscoverySet.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeIpamResourceDiscoveryAssociations
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('ipamResourceDiscoveryAssociationSet') do |node|
          children = node.children('item')
          data.ipam_resource_discovery_associations = IpamResourceDiscoveryAssociationSet.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeTransitGatewayPolicyTables
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('transitGatewayPolicyTables') do |node|
          children = node.children('item')
          data.transit_gateway_policy_tables = TransitGatewayPolicyTableList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeTransitGatewayRouteTableAnnouncements
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('transitGatewayRouteTableAnnouncements') do |node|
          children = node.children('item')
          data.transit_gateway_route_table_announcements = TransitGatewayRouteTableAnnouncementList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVerifiedAccessEndpoints
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessEndpointSet') do |node|
          children = node.children('item')
          data.verified_access_endpoints = VerifiedAccessEndpointList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVerifiedAccessGroups
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessGroupSet') do |node|
          children = node.children('item')
          data.verified_access_groups = VerifiedAccessGroupList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVerifiedAccessInstanceLoggingConfigurations
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('loggingConfigurationSet') do |node|
          children = node.children('item')
          data.logging_configurations = VerifiedAccessInstanceLoggingConfigurationList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVerifiedAccessInstances
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessInstanceSet') do |node|
          children = node.children('item')
          data.verified_access_instances = VerifiedAccessInstanceList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVerifiedAccessTrustProviders
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('verifiedAccessTrustProviderSet') do |node|
          children = node.children('item')
          data.verified_access_trust_providers = VerifiedAccessTrustProviderList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class DescribeVpcAttribute
      def self.parse(http_resp)
        ...
        xml.at('enableNetworkAddressUsageMetrics') do |node|
          data.enable_network_address_usage_metrics = AttributeBooleanValue.parse(node)
        end
        data
      end
    end
    ...
    class DnsOptions
      def self.parse(xml)
        ...
        xml.at('privateDnsOnlyForInboundResolverEndpoint') do |node|
          data.private_dns_only_for_inbound_resolver_endpoint = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class Ec2InstanceConnectEndpoint
      def self.parse(xml)
        data = Types::Ec2InstanceConnectEndpoint.new
        xml.at('ownerId') do |node|
          data.owner_id = (node.text || '')
        end
        xml.at('instanceConnectEndpointId') do |node|
          data.instance_connect_endpoint_id = (node.text || '')
        end
        xml.at('instanceConnectEndpointArn') do |node|
          data.instance_connect_endpoint_arn = (node.text || '')
        end
        xml.at('state') do |node|
          data.state = (node.text || '')
        end
        ...
        xml.at('fipsDnsName') do |node|
          data.fips_dns_name = (node.text || '')
        end
        xml.at('networkInterfaceIdSet') do |node|
          children = node.children('item')
          data.network_interface_ids = NetworkInterfaceIdSet.parse(children)
        end
        xml.at('vpcId') do |node|
          data.vpc_id = (node.text || '')
        end
        ...
        xml.at('preserveClientIp') do |node|
          data.preserve_client_ip = (node.text == 'true')
        end
        xml.at('securityGroupIdSet') do |node|
          children = node.children('item')
          data.security_group_ids = SecurityGroupIdSet.parse(children)
        end
        xml.at('tagSet') do |node|
          children = node.children('item')
          data.tags = TagList.parse(children)
    ...
    class Explanation
      def self.parse(xml)
        ...
        xml.at('componentAccount') do |node|
          data.component_account = (node.text || '')
        end
        xml.at('componentRegion') do |node|
          data.component_region = (node.text || '')
        end
        xml.at('firewallStatelessRule') do |node|
          data.firewall_stateless_rule = FirewallStatelessRule.parse(node)
        end
        xml.at('firewallStatefulRule') do |node|
          data.firewall_stateful_rule = FirewallStatefulRule.parse(node)
        end
        return data
      end
    end
    ...
    class FilterPortRange
      def self.parse(xml)
        data = Types::FilterPortRange.new
    ...
    class FirewallStatefulRule
      def self.parse(xml)
        data = Types::FirewallStatefulRule.new
        xml.at('ruleGroupArn') do |node|
          data.rule_group_arn = (node.text || '')
        end
        xml.at('sourceSet') do |node|
          children = node.children('item')
          data.sources = ValueStringList.parse(children)
        end
        xml.at('destinationSet') do |node|
          children = node.children('item')
          data.destinations = ValueStringList.parse(children)
        end
        xml.at('sourcePortSet') do |node|
          children = node.children('item')
          data.source_ports = PortRangeList.parse(children)
        end
        xml.at('destinationPortSet') do |node|
          children = node.children('item')
          data.destination_ports = PortRangeList.parse(children)
        end
        xml.at('protocol') do |node|
          data.protocol = (node.text || '')
        end
    ...
    class FirewallStatelessRule
      def self.parse(xml)
        data = Types::FirewallStatelessRule.new
        xml.at('ruleGroupArn') do |node|
          data.rule_group_arn = (node.text || '')
        end
        xml.at('sourceSet') do |node|
        ...
        xml.at('protocolSet') do |node|
          children = node.children('item')
          data.protocols = ProtocolIntList.parse(children)
        end
        xml.at('ruleAction') do |node|
          data.rule_action = (node.text || '')
        end
    ...
    class FlowLog
      def self.parse(xml)
        ...
        xml.at('deliverCrossAccountRole') do |node|
          data.deliver_cross_account_role = (node.text || '')
        end
    ...
    class FpgaImage
      def self.parse(xml)
        ...
        xml.at('instanceTypes') do |node|
          children = node.children('item')
          data.instance_types = InstanceTypesList.parse(children)
        end
        return data
      end
    end
    ...
    class GetAwsNetworkPerformanceData
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('dataResponseSet') do |node|
          children = node.children('item')
          data.data_responses = DataResponses.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class GetIpamDiscoveredResourceCidrs
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('ipamDiscoveredResourceCidrSet') do |node|
          children = node.children('item')
          data.ipam_discovered_resource_cidrs = IpamDiscoveredResourceCidrSet.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class GetTransitGatewayPolicyTableAssociations
      def self.parse(http_resp)
        ...
        xml.at('associations') do |node|
          children = node.children('item')
          data.associations = TransitGatewayPolicyTableAssociationList.parse(children)
        end
        xml.at('nextToken') do |node|
          data.next_token = (node.text || '')
        end
    ...
    class GetTransitGatewayPolicyTableEntries
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('transitGatewayPolicyTableEntries') do |node|
          children = node.children('item')
          data.transit_gateway_policy_table_entries = TransitGatewayPolicyTableEntryList.parse(children)
        end
        data
      end
    end
    ...
    class GetVerifiedAccessEndpointPolicy
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('policyEnabled') do |node|
          data.policy_enabled = (node.text == 'true')
        end
    ...
    class GetVpnTunnelReplacementStatus
      def self.parse(http_resp)
        ...
        xml.at('vpnTunnelOutsideIpAddress') do |node|
          data.vpn_tunnel_outside_ip_address = (node.text || '')
        end
        xml.at('maintenanceDetails') do |node|
          data.maintenance_details = MaintenanceDetails.parse(node)
        end
        data
      end
    end
    ...
    class Host
      def self.parse(xml)
        ...
        xml.at('hostMaintenance') do |node|
          data.host_maintenance = (node.text || '')
        end
        xml.at('assetId') do |node|
          data.asset_id = (node.text || '')
        end
        return data
      end
    end
    ...
    class InferenceAcceleratorInfo
      def self.parse(xml)
        ...
        xml.at('totalInferenceMemoryInMiB') do |node|
          data.total_inference_memory_in_mi_b = node.text&.to_i
        end
        return data
      end
    end
    ...
    class InferenceDeviceInfo
      def self.parse(xml)
        ...
        xml.at('memoryInfo') do |node|
          data.memory_info = InferenceDeviceMemoryInfo.parse(node)
        end
        return data
      end
    end
    ...
    class InferenceDeviceMemoryInfo
      def self.parse(xml)
        data = Types::InferenceDeviceMemoryInfo.new
        xml.at('sizeInMiB') do |node|
          data.size_in_mi_b = node.text&.to_i
        end
        return data
    ...
    class Instance
      def self.parse(xml)
        ...
        xml.at('currentInstanceBootMode') do |node|
          data.current_instance_boot_mode = (node.text || '')
        end
        return data
      end
    end
    ...
    class InstanceConnectEndpointSet
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << Ec2InstanceConnectEndpoint.parse(node)
        end
        data
      end
    end
    ...
    class InstanceRequirements
      def self.parse(xml)
        ...
        xml.at('networkBandwidthGbps') do |node|
          data.network_bandwidth_gbps = NetworkBandwidthGbps.parse(node)
        end
        xml.at('allowedInstanceTypeSet') do |node|
          children = node.children('item')
    ...
    class InstanceTypeInfo
      def self.parse(xml)
        ...
        xml.at('nitroEnclavesSupport') do |node|
          data.nitro_enclaves_support = (node.text || '')
        end
        xml.at('nitroTpmSupport') do |node|
          data.nitro_tpm_support = (node.text || '')
        end
        xml.at('nitroTpmInfo') do |node|
          data.nitro_tpm_info = NitroTpmInfo.parse(node)
        end
        return data
      end
    end
    ...
    class Ipam
      def self.parse(xml)
        ...
        xml.at('defaultResourceDiscoveryId') do |node|
          data.default_resource_discovery_id = (node.text || '')
        end
        xml.at('defaultResourceDiscoveryAssociationId') do |node|
          data.default_resource_discovery_association_id = (node.text || '')
        end
    ...
    class IpamDiscoveredAccount
      def self.parse(xml)
        data = Types::IpamDiscoveredAccount.new
        xml.at('accountId') do |node|
          data.account_id = (node.text || '')
        end
        xml.at('discoveryRegion') do |node|
          data.discovery_region = (node.text || '')
        end
        xml.at('failureReason') do |node|
          data.failure_reason = IpamDiscoveryFailureReason.parse(node)
        end
        xml.at('lastAttemptedDiscoveryTime') do |node|
          data.last_attempted_discovery_time = Time.parse(node.text) if node.text
        end
        xml.at('lastSuccessfulDiscoveryTime') do |node|
          data.last_successful_discovery_time = Time.parse(node.text) if node.text
        end
        return data
      end
    end
    ...
    class IpamDiscoveredResourceCidr
      def self.parse(xml)
        data = Types::IpamDiscoveredResourceCidr.new
        xml.at('ipamResourceDiscoveryId') do |node|
          data.ipam_resource_discovery_id = (node.text || '')
        end
        xml.at('resourceRegion') do |node|
    ...
    class IpamDiscoveredResourceCidrSet
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << IpamDiscoveredResourceCidr.parse(node)
        end
        data
      end
    end
    ...
    class IpamDiscoveryFailureReason
      def self.parse(xml)
        data = Types::IpamDiscoveryFailureReason.new
        xml.at('code') do |node|
          data.code = (node.text || '')
        end
        xml.at('message') do |node|
    ...
    class IpamPoolCidr
      def self.parse(xml)
        ...
        xml.at('ipamPoolCidrId') do |node|
          data.ipam_pool_cidr_id = (node.text || '')
        end
        xml.at('netmaskLength') do |node|
          data.netmask_length = node.text&.to_i
        end
        return data
      end
    end
    ...
    class IpamResourceDiscovery
      def self.parse(xml)
        data = Types::IpamResourceDiscovery.new
        xml.at('ownerId') do |node|
          data.owner_id = (node.text || '')
        end
        xml.at('ipamResourceDiscoveryId') do |node|
        ...
        xml.at('ipamResourceDiscoveryArn') do |node|
          data.ipam_resource_discovery_arn = (node.text || '')
        end
        xml.at('ipamResourceDiscoveryRegion') do |node|
          data.ipam_resource_discovery_region = (node.text || '')
        end
        xml.at('description') do |node|
          data.description = (node.text || '')
        end
    ...
    class IpamResourceDiscoveryAssociation
      def self.parse(xml)
        data = Types::IpamResourceDiscoveryAssociation.new
        xml.at('ownerId') do |node|
          data.owner_id = (node.text || '')
        end
        xml.at('ipamResourceDiscoveryAssociationId') do |node|
          data.ipam_resource_discovery_association_id = (node.text || '')
        end
        xml.at('ipamResourceDiscoveryAssociationArn') do |node|
          data.ipam_resource_discovery_association_arn = (node.text || '')
        end
        xml.at('ipamResourceDiscoveryId') do |node|
          data.ipam_resource_discovery_id = (node.text || '')
        end
        ...
        xml.at('resourceDiscoveryStatus') do |node|
          data.resource_discovery_status = (node.text || '')
        end
        xml.at('state') do |node|
          data.state = (node.text || '')
        end
    ...
    class IpamResourceDiscoveryAssociationSet
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << IpamResourceDiscoveryAssociation.parse(node)
        end
        data
      end
    end
    ...
    class IpamResourceDiscoverySet
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << IpamResourceDiscovery.parse(node)
        end
        data
      end
    end
    ...
    class MaintenanceDetails
      def self.parse(xml)
        data = Types::MaintenanceDetails.new
        xml.at('pendingMaintenance') do |node|
          data.pending_maintenance = (node.text || '')
        end
        xml.at('maintenanceAutoAppliedAfter') do |node|
          data.maintenance_auto_applied_after = Time.parse(node.text) if node.text
        end
        xml.at('lastMaintenanceApplied') do |node|
          data.last_maintenance_applied = Time.parse(node.text) if node.text
        end
        return data
      end
    end
    ...
    class MetricPoint
      def self.parse(xml)
        ...
        xml.at('value') do |node|
          data.value = Hearth::NumberHelper.deserialize(node.text)
        end
        xml.at('status') do |node|
          data.status = (node.text || '')
        end
    ...
    class ModifyVerifiedAccessInstanceLoggingConfiguration
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('loggingConfiguration') do |node|
          data.logging_configuration = VerifiedAccessInstanceLoggingConfiguration.parse(node)
        end
        data
      end
    end
    ...
    class ModifyVpcEndpointServicePermissions
      def self.parse(http_resp)
        ...
        return data if body.empty?
        xml = Hearth::XML.parse(body)
        xml.at('addedPrincipalSet') do |node|
          children = node.children('item')
          data.added_principals = AddedPrincipalSet.parse(children)
        end
        xml.at('return') do |node|
          data.return_value = (node.text == 'true')
        end
    ...
    class NatGatewayAddress
      def self.parse(xml)
        ...
        xml.at('isPrimary') do |node|
          data.is_primary = (node.text == 'true')
        end
        xml.at('failureMessage') do |node|
          data.failure_message = (node.text || '')
        end
    ...
    class NetworkCardInfo
      def self.parse(xml)
        ...
        xml.at('baselineBandwidthInGbps') do |node|
          data.baseline_bandwidth_in_gbps = Hearth::NumberHelper.deserialize(node.text)
        end
        xml.at('peakBandwidthInGbps') do |node|
          data.peak_bandwidth_in_gbps = Hearth::NumberHelper.deserialize(node.text)
        end
        return data
      end
    end
    ...
    class NetworkInfo
      def self.parse(xml)
        ...
        xml.at('enaSrdSupported') do |node|
          data.ena_srd_supported = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class NetworkInsightsAnalysis
      def self.parse(xml)
        ...
        xml.at('additionalAccountSet') do |node|
          children = node.children('item')
          data.additional_accounts = ValueStringList.parse(children)
        end
        ...
        xml.at('suggestedAccountSet') do |node|
          children = node.children('item')
          data.suggested_accounts = ValueStringList.parse(children)
        end
        xml.at('tagSet') do |node|
          children = node.children('item')
          data.tags = TagList.parse(children)
    ...
    class NetworkInsightsPath
      def self.parse(xml)
        ...
        xml.at('destinationArn') do |node|
          data.destination_arn = (node.text || '')
        end
        ...
        xml.at('filterAtSource') do |node|
          data.filter_at_source = PathFilter.parse(node)
        end
        xml.at('filterAtDestination') do |node|
          data.filter_at_destination = PathFilter.parse(node)
        end
        return data
      end
    end
    ...
    class NetworkInterfaceAttachment
      def self.parse(xml)
        ...
        xml.at('enaSrdSpecification') do |node|
          data.ena_srd_specification = AttachmentEnaSrdSpecification.parse(node)
        end
        return data
      end
    end
    ...
    class NitroTpmInfo
      def self.parse(xml)
        data = Types::NitroTpmInfo.new
        xml.at('supportedVersions') do |node|
          children = node.children('item')
          data.supported_versions = NitroTpmSupportedVersionsList.parse(children)
        end
        return data
      end
    end
    ...
    class PathFilter
      def self.parse(xml)
        data = Types::PathFilter.new
        xml.at('sourceAddress') do |node|
          data.source_address = (node.text || '')
        end
        xml.at('sourcePortRange') do |node|
          data.source_port_range = FilterPortRange.parse(node)
        end
        xml.at('destinationAddress') do |node|
          data.destination_address = (node.text || '')
        end
        xml.at('destinationPortRange') do |node|
          data.destination_port_range = FilterPortRange.parse(node)
        end
        return data
      end
    end
    ...
    class PeeringTgwInfo
      def self.parse(xml)
        ...
        xml.at('coreNetworkId') do |node|
          data.core_network_id = (node.text || '')
        end
        xml.at('ownerId') do |node|
          data.owner_id = (node.text || '')
        end
    ...
    class PlacementGroup
      def self.parse(xml)
        ...
        xml.at('spreadLevel') do |node|
          data.spread_level = (node.text || '')
        end
        return data
      end
    end
    ...
    class ProcessorInfo
      def self.parse(xml)
        ...
        xml.at('supportedFeatures') do |node|
          children = node.children('item')
    ...
    class ReplaceRootVolumeTask
      def self.parse(xml)
        ...
        xml.at('deleteReplacedRootVolume') do |node|
          data.delete_replaced_root_volume = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class RuleGroupRuleOptionsPair
      def self.parse(xml)
        data = Types::RuleGroupRuleOptionsPair.new
        xml.at('ruleGroupArn') do |node|
          data.rule_group_arn = (node.text || '')
        end
        xml.at('ruleOptionSet') do |node|
    ...
    class RuleGroupRuleOptionsPairList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << RuleGroupRuleOptionsPair.parse(node)
        end
        data
      end
    end
    ...
    class RuleGroupTypePair
      def self.parse(xml)
        data = Types::RuleGroupTypePair.new
        xml.at('ruleGroupArn') do |node|
          data.rule_group_arn = (node.text || '')
        end
        xml.at('ruleGroupType') do |node|
          data.rule_group_type = (node.text || '')
        end
        return data
      end
    end
    ...
    class RuleGroupTypePairList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << RuleGroupTypePair.parse(node)
        end
        data
      end
    end
    ...
    class RuleOption
      def self.parse(xml)
        ...
        xml.at('keyword') do |node|
          data.keyword = (node.text || '')
        end
        xml.at('settingSet') do |node|
          children = node.children('item')
          data.settings = StringList.parse(children)
        end
        return data
      end
    end
    ...
    class TransitGatewayPeeringAttachment
      def self.parse(xml)
        ...
        xml.at('accepterTransitGatewayAttachmentId') do |node|
          data.accepter_transit_gateway_attachment_id = (node.text || '')
        end
        ...
        xml.at('options') do |node|
          data.options = TransitGatewayPeeringAttachmentOptions.parse(node)
        end
        xml.at('status') do |node|
    ...
    class TransitGatewayPeeringAttachmentOptions
      def self.parse(xml)
        data = Types::TransitGatewayPeeringAttachmentOptions.new
        xml.at('dynamicRouting') do |node|
          data.dynamic_routing = (node.text || '')
        end
        return data
      end
    end
    ...
    class TransitGatewayPolicyRule
      def self.parse(xml)
        data = Types::TransitGatewayPolicyRule.new
        xml.at('sourceCidrBlock') do |node|
          data.source_cidr_block = (node.text || '')
        end
        xml.at('sourcePortRange') do |node|
          data.source_port_range = (node.text || '')
        end
        xml.at('destinationCidrBlock') do |node|
          data.destination_cidr_block = (node.text || '')
        end
        xml.at('destinationPortRange') do |node|
          data.destination_port_range = (node.text || '')
        end
        xml.at('protocol') do |node|
          data.protocol = (node.text || '')
        end
        xml.at('metaData') do |node|
          data.meta_data = TransitGatewayPolicyRuleMetaData.parse(node)
        end
        return data
      end
    end
    ...
    class TransitGatewayPolicyRuleMetaData
      def self.parse(xml)
        data = Types::TransitGatewayPolicyRuleMetaData.new
        xml.at('metaDataKey') do |node|
          data.meta_data_key = (node.text || '')
        end
        xml.at('metaDataValue') do |node|
          data.meta_data_value = (node.text || '')
        end
        return data
      end
    end
    ...
    class TransitGatewayPolicyTable
      def self.parse(xml)
        data = Types::TransitGatewayPolicyTable.new
        xml.at('transitGatewayPolicyTableId') do |node|
          data.transit_gateway_policy_table_id = (node.text || '')
        end
        xml.at('transitGatewayId') do |node|
          data.transit_gateway_id = (node.text || '')
        end
    ...
    class TransitGatewayPolicyTableAssociation
      def self.parse(xml)
        data = Types::TransitGatewayPolicyTableAssociation.new
        xml.at('transitGatewayPolicyTableId') do |node|
          data.transit_gateway_policy_table_id = (node.text || '')
        end
        xml.at('transitGatewayAttachmentId') do |node|
    ...
    class TransitGatewayPolicyTableAssociationList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << TransitGatewayPolicyTableAssociation.parse(node)
        end
        data
      end
    end
    ...
    class TransitGatewayPolicyTableEntry
      def self.parse(xml)
        data = Types::TransitGatewayPolicyTableEntry.new
        xml.at('policyRuleNumber') do |node|
          data.policy_rule_number = (node.text || '')
        end
        xml.at('policyRule') do |node|
          data.policy_rule = TransitGatewayPolicyRule.parse(node)
        end
        xml.at('targetRouteTableId') do |node|
          data.target_route_table_id = (node.text || '')
        end
        return data
      end
    end
    ...
    class TransitGatewayPolicyTableEntryList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << TransitGatewayPolicyTableEntry.parse(node)
        end
        data
      end
    end
    ...
    class TransitGatewayPolicyTableList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << TransitGatewayPolicyTable.parse(node)
        end
        data
      end
    end
    ...
    class TransitGatewayPropagation
      def self.parse(xml)
        ...
        xml.at('transitGatewayRouteTableAnnouncementId') do |node|
          data.transit_gateway_route_table_announcement_id = (node.text || '')
        end
        return data
      end
    end
    ...
    class TransitGatewayRouteTableAnnouncement
      def self.parse(xml)
        data = Types::TransitGatewayRouteTableAnnouncement.new
        xml.at('transitGatewayRouteTableAnnouncementId') do |node|
          data.transit_gateway_route_table_announcement_id = (node.text || '')
        end
        xml.at('transitGatewayId') do |node|
        ...
        xml.at('peerTransitGatewayId') do |node|
          data.peer_transit_gateway_id = (node.text || '')
        end
        xml.at('peerCoreNetworkId') do |node|
          data.peer_core_network_id = (node.text || '')
        end
        xml.at('peeringAttachmentId') do |node|
          data.peering_attachment_id = (node.text || '')
        end
        xml.at('announcementDirection') do |node|
          data.announcement_direction = (node.text || '')
        end
        xml.at('transitGatewayRouteTableId') do |node|
          data.transit_gateway_route_table_id = (node.text || '')
        end
    ...
    class TransitGatewayRouteTableAnnouncementList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << TransitGatewayRouteTableAnnouncement.parse(node)
        end
        data
      end
    end
    ...
    class TunnelOption
      def self.parse(xml)
        ...
        xml.at('logOptions') do |node|
          data.log_options = VpnTunnelLogOptions.parse(node)
        end
        xml.at('enableTunnelLifecycleControl') do |node|
          data.enable_tunnel_lifecycle_control = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class VerifiedAccessEndpoint
      def self.parse(xml)
        data = Types::VerifiedAccessEndpoint.new
        xml.at('verifiedAccessInstanceId') do |node|
          data.verified_access_instance_id = (node.text || '')
        end
        xml.at('verifiedAccessGroupId') do |node|
          data.verified_access_group_id = (node.text || '')
        end
        xml.at('verifiedAccessEndpointId') do |node|
          data.verified_access_endpoint_id = (node.text || '')
        end
        xml.at('applicationDomain') do |node|
          data.application_domain = (node.text || '')
        end
        xml.at('endpointType') do |node|
          data.endpoint_type = (node.text || '')
        end
        xml.at('attachmentType') do |node|
          data.attachment_type = (node.text || '')
        end
        xml.at('domainCertificateArn') do |node|
          data.domain_certificate_arn = (node.text || '')
        end
        xml.at('endpointDomain') do |node|
          data.endpoint_domain = (node.text || '')
        end
        xml.at('deviceValidationDomain') do |node|
          data.device_validation_domain = (node.text || '')
        end
        xml.at('securityGroupIdSet') do |node|
          children = node.children('item')
          data.security_group_ids = SecurityGroupIdList.parse(children)
        end
        xml.at('loadBalancerOptions') do |node|
          data.load_balancer_options = VerifiedAccessEndpointLoadBalancerOptions.parse(node)
        end
        xml.at('networkInterfaceOptions') do |node|
          data.network_interface_options = VerifiedAccessEndpointEniOptions.parse(node)
        end
        xml.at('status') do |node|
          data.status = VerifiedAccessEndpointStatus.parse(node)
        end
        xml.at('description') do |node|
          data.description = (node.text || '')
        end
        ...
        xml.at('lastUpdatedTime') do |node|
          data.last_updated_time = (node.text || '')
        end
        xml.at('deletionTime') do |node|
          data.deletion_time = (node.text || '')
        end
    ...
    class VerifiedAccessEndpointEniOptions
      def self.parse(xml)
        data = Types::VerifiedAccessEndpointEniOptions.new
        xml.at('networkInterfaceId') do |node|
          data.network_interface_id = (node.text || '')
        end
        xml.at('protocol') do |node|
    ...
    class VerifiedAccessEndpointList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessEndpoint.parse(node)
        end
        data
      end
    end
    ...
    class VerifiedAccessEndpointLoadBalancerOptions
      def self.parse(xml)
        data = Types::VerifiedAccessEndpointLoadBalancerOptions.new
        xml.at('protocol') do |node|
          data.protocol = (node.text || '')
        end
        xml.at('port') do |node|
        ...
        xml.at('subnetIdSet') do |node|
          children = node.children('item')
          data.subnet_ids = VerifiedAccessEndpointSubnetIdList.parse(children)
        end
        return data
      end
    end
    ...
    class VerifiedAccessEndpointStatus
      def self.parse(xml)
        data = Types::VerifiedAccessEndpointStatus.new
        xml.at('code') do |node|
          data.code = (node.text || '')
        end
        xml.at('message') do |node|
    ...
    class VerifiedAccessGroup
      def self.parse(xml)
        data = Types::VerifiedAccessGroup.new
        xml.at('verifiedAccessGroupId') do |node|
          data.verified_access_group_id = (node.text || '')
        end
        xml.at('verifiedAccessInstanceId') do |node|
        ...
        xml.at('verifiedAccessGroupArn') do |node|
          data.verified_access_group_arn = (node.text || '')
        end
        xml.at('creationTime') do |node|
          data.creation_time = (node.text || '')
        end
    ...
    class VerifiedAccessGroupList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessGroup.parse(node)
        end
        data
      end
    end
    ...
    class VerifiedAccessInstance
      def self.parse(xml)
        data = Types::VerifiedAccessInstance.new
        xml.at('verifiedAccessInstanceId') do |node|
          data.verified_access_instance_id = (node.text || '')
        end
        xml.at('description') do |node|
        ...
        xml.at('verifiedAccessTrustProviderSet') do |node|
          children = node.children('item')
          data.verified_access_trust_providers = VerifiedAccessTrustProviderCondensedList.parse(children)
        end
        xml.at('creationTime') do |node|
          data.creation_time = (node.text || '')
        end
    ...
    class VerifiedAccessInstanceList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessInstance.parse(node)
        end
        data
      end
    end
    ...
    class VerifiedAccessInstanceLoggingConfiguration
      def self.parse(xml)
        data = Types::VerifiedAccessInstanceLoggingConfiguration.new
        xml.at('verifiedAccessInstanceId') do |node|
          data.verified_access_instance_id = (node.text || '')
        end
        xml.at('accessLogs') do |node|
          data.access_logs = VerifiedAccessLogs.parse(node)
        end
        return data
      end
    end
    ...
    class VerifiedAccessInstanceLoggingConfigurationList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessInstanceLoggingConfiguration.parse(node)
        end
        data
      end
    end
    ...
    class VerifiedAccessLogCloudWatchLogsDestination
      def self.parse(xml)
        data = Types::VerifiedAccessLogCloudWatchLogsDestination.new
        xml.at('enabled') do |node|
          data.enabled = (node.text == 'true')
        end
        xml.at('deliveryStatus') do |node|
          data.delivery_status = VerifiedAccessLogDeliveryStatus.parse(node)
        end
    ...
    class VerifiedAccessLogDeliveryStatus
      def self.parse(xml)
        data = Types::VerifiedAccessLogDeliveryStatus.new
        xml.at('code') do |node|
          data.code = (node.text || '')
        end
        xml.at('message') do |node|
    ...
    class VerifiedAccessLogKinesisDataFirehoseDestination
      def self.parse(xml)
        data = Types::VerifiedAccessLogKinesisDataFirehoseDestination.new
        xml.at('enabled') do |node|
          data.enabled = (node.text == 'true')
        end
        xml.at('deliveryStatus') do |node|
    ...
    class VerifiedAccessLogS3Destination
      def self.parse(xml)
        data = Types::VerifiedAccessLogS3Destination.new
        xml.at('enabled') do |node|
          data.enabled = (node.text == 'true')
        end
        xml.at('deliveryStatus') do |node|
        ...
        xml.at('bucketOwner') do |node|
          data.bucket_owner = (node.text || '')
        end
        return data
      end
    end
    ...
    class VerifiedAccessLogs
      def self.parse(xml)
        data = Types::VerifiedAccessLogs.new
        xml.at('s3') do |node|
          data.s3 = VerifiedAccessLogS3Destination.parse(node)
        end
        xml.at('cloudWatchLogs') do |node|
          data.cloud_watch_logs = VerifiedAccessLogCloudWatchLogsDestination.parse(node)
        end
        xml.at('kinesisDataFirehose') do |node|
          data.kinesis_data_firehose = VerifiedAccessLogKinesisDataFirehoseDestination.parse(node)
        end
        xml.at('logVersion') do |node|
          data.log_version = (node.text || '')
        end
        xml.at('includeTrustContext') do |node|
          data.include_trust_context = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class VerifiedAccessTrustProvider
      def self.parse(xml)
        ...
        xml.at('verifiedAccessTrustProviderId') do |node|
          data.verified_access_trust_provider_id = (node.text || '')
        end
        xml.at('description') do |node|
          data.description = (node.text || '')
        end
        xml.at('trustProviderType') do |node|
          data.trust_provider_type = (node.text || '')
        end
        xml.at('userTrustProviderType') do |node|
          data.user_trust_provider_type = (node.text || '')
        end
        xml.at('deviceTrustProviderType') do |node|
          data.device_trust_provider_type = (node.text || '')
        end
        xml.at('oidcOptions') do |node|
          data.oidc_options = OidcOptions.parse(node)
        end
        xml.at('deviceOptions') do |node|
          data.device_options = DeviceOptions.parse(node)
        end
        xml.at('policyReferenceName') do |node|
          data.policy_reference_name = (node.text || '')
        end
        xml.at('creationTime') do |node|
          data.creation_time = (node.text || '')
        end
    ...
    class VerifiedAccessTrustProviderCondensed
      def self.parse(xml)
        data = Types::VerifiedAccessTrustProviderCondensed.new
        xml.at('verifiedAccessTrustProviderId') do |node|
          data.verified_access_trust_provider_id = (node.text || '')
        end
        xml.at('description') do |node|
    ...
    class VerifiedAccessTrustProviderCondensedList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessTrustProviderCondensed.parse(node)
        end
        data
      end
    end
    ...
    class VerifiedAccessTrustProviderList
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << VerifiedAccessTrustProvider.parse(node)
        end
        data
      end
    end
    ...
    class VpcEndpointConnection
      def self.parse(xml)
        ...
        xml.at('vpcEndpointConnectionId') do |node|
          data.vpc_endpoint_connection_id = (node.text || '')
        end
        xml.at('tagSet') do |node|
          children = node.children('item')
          data.tags = TagList.parse(children)
    ...
    class VpnConnectionOptions
      def self.parse(xml)
        ...
        xml.at('outsideIpAddressType') do |node|
          data.outside_ip_address_type = (node.text || '')
        end
        xml.at('transportTransitGatewayAttachmentId') do |node|
          data.transport_transit_gateway_attachment_id = (node.text || '')
        end
    ...
    class VpnTunnelLogOptions
      def self.parse(xml)
        data = Types::VpnTunnelLogOptions.new
        xml.at('cloudWatchLogOptions') do |node|
          data.cloud_watch_log_options = CloudWatchLogOptions.parse(node)
        end
        return data
      end
    end
