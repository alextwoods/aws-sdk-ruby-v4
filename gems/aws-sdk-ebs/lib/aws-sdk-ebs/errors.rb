...
module AWS::SDK::EBS
  module Errors
    ...
    class ConcurrentLimitExceededException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConcurrentLimitExceededException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConcurrentLimitExceededException]
      #
      attr_reader :data
    end
    ...
    class RequestThrottledException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::RequestThrottledException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::RequestThrottledException]
      #
      attr_reader :data
    end
