  ...
  module Parsers
    ...
    class CompositeAlarm
      def self.parse(xml)
        ...
        xml.at('StateTransitionedTimestamp') do |node|
          data.state_transitioned_timestamp = Time.parse(node.text) if node.text
        end
        xml.at('ActionsSuppressedBy') do |node|
          data.actions_suppressed_by = (node.text || '')
        end
        xml.at('ActionsSuppressedReason') do |node|
          data.actions_suppressed_reason = (node.text || '')
        end
        xml.at('ActionsSuppressor') do |node|
          data.actions_suppressor = (node.text || '')
        end
        xml.at('ActionsSuppressorWaitPeriod') do |node|
          data.actions_suppressor_wait_period = node.text&.to_i
        end
        xml.at('ActionsSuppressorExtensionPeriod') do |node|
          data.actions_suppressor_extension_period = node.text&.to_i
        end
        return data
      end
    end
    ...
    class InsightRule
      def self.parse(xml)
        ...
        xml.at('ManagedRule') do |node|
          data.managed_rule = (node.text == 'true')
        end
        return data
      end
    end
    ...
    class ManagedRuleDescription
      def self.parse(xml)
        data = Types::ManagedRuleDescription.new
        ...
        xml.at('RuleState') do |node|
          data.rule_state = ManagedRuleState.parse(node)
        end
        return data
      end
    end
    ...
    class ManagedRuleDescriptions
      def self.parse(xml)
        data = []
        xml.each do |node|
          data << ManagedRuleDescription.parse(node)
        end
        data
      end
    end
    ...
    class ManagedRuleState
      def self.parse(xml)
        data = Types::ManagedRuleState.new
        xml.at('RuleName') do |node|
          data.rule_name = (node.text || '')
        end
        xml.at('State') do |node|
          data.state = (node.text || '')
        end
    ...
    class MetricAlarm
      def self.parse(xml)
        ...
        xml.at('EvaluationState') do |node|
          data.evaluation_state = (node.text || '')
        end
        xml.at('StateTransitionedTimestamp') do |node|
          data.state_transitioned_timestamp = Time.parse(node.text) if node.text
        end
    ...
    class MetricStreamFilter
      def self.parse(xml)
        ...
        xml.at('MetricNames') do |node|
          children = node.children('member')
          data.metric_names = MetricStreamFilterMetricNames.parse(children)
        end
        return data
      end
    end
