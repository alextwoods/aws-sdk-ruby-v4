...
module AWS::SDK::ApiGatewayManagementApi
  module Errors
    ...
    class GoneException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::GoneException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)
    ...
    class PayloadTooLargeException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::PayloadTooLargeException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)
