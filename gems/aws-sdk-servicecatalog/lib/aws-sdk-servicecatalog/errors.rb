...
module AWS::SDK::ServiceCatalog
  module Errors
    ...
    class InvalidParametersException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::InvalidParametersException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::InvalidParametersException]
      #
      attr_reader :data
    end
    ...
    class TagOptionNotMigratedException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::TagOptionNotMigratedException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::TagOptionNotMigratedException]
      #
      attr_reader :data
    end
