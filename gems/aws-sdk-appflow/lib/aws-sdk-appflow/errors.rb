...
module AWS::SDK::Appflow
  module Errors
    ...
    class ConnectorAuthenticationException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConnectorAuthenticationException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConnectorAuthenticationException]
      #
      attr_reader :data
    end

    class ConnectorServerException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConnectorServerException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConnectorServerException]
      #
      attr_reader :data
    end
