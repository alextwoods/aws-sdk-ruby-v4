...
module AWS::SDK::LexRuntimeService
  module Errors
    ...
    class LoopDetectedException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::LoopDetectedException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::LoopDetectedException]
      #
      attr_reader :data
    end

    class NotAcceptableException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::NotAcceptableException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::NotAcceptableException]
      #
      attr_reader :data
    end
