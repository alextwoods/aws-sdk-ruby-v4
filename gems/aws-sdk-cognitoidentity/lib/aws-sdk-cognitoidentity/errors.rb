...
module AWS::SDK::CognitoIdentity
  module Errors
    ...
    class DeveloperUserAlreadyRegisteredException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DeveloperUserAlreadyRegisteredException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DeveloperUserAlreadyRegisteredException]
      #
      attr_reader :data
    end

    class ExternalServiceException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ExternalServiceException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ExternalServiceException]
      #
      attr_reader :data
    end
    ...
    class InvalidIdentityPoolConfigurationException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::InvalidIdentityPoolConfigurationException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::InvalidIdentityPoolConfigurationException]
      #
      attr_reader :data
    end
