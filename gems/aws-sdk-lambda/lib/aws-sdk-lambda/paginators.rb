...
module AWS::SDK::Lambda
  module Paginators
    ...
    class ListFunctionUrlConfigs
      # @param [Client] client
      # @param [Hash] params (see Client#list_function_url_configs)
      # @param [Hash] options (see Client#list_function_url_configs)
      def initialize(client, params = {}, options = {})
        @params = params
        @options = options
        @client = client
      end
      # Iterate all response pages of the list_function_url_configs operation.
      # @return [Enumerator]
      def pages
        params = @params
        Enumerator.new do |e|
          @prev_token = params[:marker]
          response = @client.list_function_url_configs(params, @options)
          e.yield(response)
          output_token = response.next_marker
      ...
      # Iterate all items from pages in the list_function_url_configs operation.
      # @return [Enumerator]
      def items
        Enumerator.new do |e|
          pages.each do |page|
            page.function_url_configs.each do |item|
              e.yield(item)
            end
          end
        end
