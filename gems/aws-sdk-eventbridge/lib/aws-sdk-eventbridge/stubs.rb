...
module AWS::SDK::EventBridge
  module Stubs
    ...
    # List Stubber for EndpointEventBusList
    ...
    # Structure Stubber for EndpointEventBus
    class EndpointEventBus
      def self.default(visited=[])
        return nil if visited.include?('EndpointEventBus')
        visited = visited + ['EndpointEventBus']
        {
      ...
      def self.stub(stub)
        stub ||= Types::EndpointEventBus.new
        data = {}
    ...
    # Structure Stubber for ReplicationConfig
    class ReplicationConfig
      def self.default(visited=[])
        return nil if visited.include?('ReplicationConfig')
        visited = visited + ['ReplicationConfig']
        {
          state: 'state',
        }
      end
      ...
      def self.stub(stub)
        stub ||= Types::ReplicationConfig.new
        data = {}
        data['State'] = stub[:state] unless stub[:state].nil?
        data
      end
    ...
    # Structure Stubber for RoutingConfig
    class RoutingConfig
      def self.default(visited=[])
    ...
    # Structure Stubber for Secondary
    class Secondary
      def self.default(visited=[])
        return nil if visited.include?('Secondary')
        visited = visited + ['Secondary']
        {
          route: 'route',
        }
      end
      ...
      def self.stub(stub)
        stub ||= Types::Secondary.new
        data = {}
        data['Route'] = stub[:route] unless stub[:route].nil?
        data
      end
    end

    # Structure Stubber for Primary
    class Primary
      def self.default(visited=[])
        return nil if visited.include?('Primary')
        visited = visited + ['Primary']
        {
      ...
      def self.stub(stub)
        stub ||= Types::Primary.new
        data = {}
        data['HealthCheck'] = stub[:health_check] unless stub[:health_check].nil?
        data
      end
    end
