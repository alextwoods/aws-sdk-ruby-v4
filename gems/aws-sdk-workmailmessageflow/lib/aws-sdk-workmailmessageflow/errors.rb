...
module AWS::SDK::WorkMailMessageFlow
  module Errors
    ...
    class InvalidContentLocation < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::InvalidContentLocation.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::InvalidContentLocation]
      #
      attr_reader :data
    end

    class MessageFrozen < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::MessageFrozen.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::MessageFrozen]
      #
      attr_reader :data
    end
