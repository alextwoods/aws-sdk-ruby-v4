          ...
          if (partition_result = AWS::SDK::Core::EndpointRules.partition(region))
            if use_fips == true && use_dual_stack == true
              if true == partition_result['supportsFIPS'] && true == partition_result['supportsDualStack']
                return Hearth::EndpointRules::Endpoint.new(uri: "https://portal.sso-fips.#{region}.#{partition_result['dualStackDnsSuffix']}")
              end
            ...
            if use_fips == true
              if partition_result['supportsFIPS'] == true
                if partition_result['name'] == "aws-us-gov"
                  return Hearth::EndpointRules::Endpoint.new(uri: "https://portal.sso.#{region}.amazonaws.com")
                end
                return Hearth::EndpointRules::Endpoint.new(uri: "https://portal.sso-fips.#{region}.#{partition_result['dnsSuffix']}")
              end
              raise ArgumentError, "FIPS is enabled but this partition does not support FIPS"
            end
            if use_dual_stack == true
              if true == partition_result['supportsDualStack']
                return Hearth::EndpointRules::Endpoint.new(uri: "https://portal.sso.#{region}.#{partition_result['dualStackDnsSuffix']}")
              end
              raise ArgumentError, "DualStack is enabled but this partition does not support DualStack"
            end
            return Hearth::EndpointRules::Endpoint.new(uri: "https://portal.sso.#{region}.#{partition_result['dnsSuffix']}")
          end
        end
