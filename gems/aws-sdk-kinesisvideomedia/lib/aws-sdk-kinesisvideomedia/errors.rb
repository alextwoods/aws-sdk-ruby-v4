...
module AWS::SDK::KinesisVideoMedia
  module Errors
    ...
    class ConnectionLimitExceededException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConnectionLimitExceededException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConnectionLimitExceededException]
      #
      attr_reader :data
    end
