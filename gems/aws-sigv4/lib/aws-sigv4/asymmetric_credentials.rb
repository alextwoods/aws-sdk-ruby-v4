  ...
  module Sigv4
    ...
    module AsymmetricCredentials
      ...
      def self.derive_asymmetric_key(access_key_id, secret_access_key)
        ...
        loop do
          kdf_context = access_key_id.unpack('C*') +
                        [counter].pack('C').unpack('C')
          input = label.unpack('C*') + [0x00] + kdf_context +
                  [bit_len].pack('L>').unpack('CCCC')
          k0 = OpenSSL::HMAC.digest(
            'SHA256', input_key, ([0, 0, 0, 0x01] + input).pack('C*')
          )
        ...
        # pk_x and pk_y are not needed for signature,
        # but useful in verification/testing
        pk_b = public_key.to_octet_string(:uncompressed)
                         .unpack('C*')
      ...
      def self.generate_ec(public_key, private_key)
        # format reversed from: OpenSSL::ASN1.decode_all(
        # OpenSSL::PKey::EC.new.to_der)
      ...
      def self.check_openssl_support!
        ...
        raise 'Sigv4a Asymmetric Credential derivation requires ' \
              'jruby-openssl >= 0.15'
      end
    end
  end
