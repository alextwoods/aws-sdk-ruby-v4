...
module AWS::SDK::Honeycode
  module Errors
    ...
    class AutomationExecutionException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::AutomationExecutionException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::AutomationExecutionException]
      #
      attr_reader :data
    end

    class AutomationExecutionTimeoutException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::AutomationExecutionTimeoutException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::AutomationExecutionTimeoutException]
      #
      attr_reader :data
    end
    ...
    class RequestTimeoutException < ApiServerError
      def initialize(http_resp:, **kwargs)
      ...
      # @return [Types::RequestTimeoutException]
      #
      attr_reader :data
    end
