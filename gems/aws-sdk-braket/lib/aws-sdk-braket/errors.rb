...
module AWS::SDK::Braket
  module Errors
    ...
    class DeviceOfflineException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DeviceOfflineException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DeviceOfflineException]
      #
      attr_reader :data
    end

    class DeviceRetiredException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DeviceRetiredException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DeviceRetiredException]
      #
      attr_reader :data
    end
