...
module AWS::SDK::Budgets
  module Errors
    ...
    class CreationLimitExceededException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::CreationLimitExceededException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::CreationLimitExceededException]
      #
      attr_reader :data
    end

    class DuplicateRecordException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DuplicateRecordException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DuplicateRecordException]
      #
      attr_reader :data
    end
    ...
    class ResourceLockedException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ResourceLockedException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ResourceLockedException]
      #
      attr_reader :data
    end
