...
module AWS::SDK::Route53RecoveryCluster
  module Errors
    ...
    class EndpointTemporarilyUnavailableException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::EndpointTemporarilyUnavailableException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::EndpointTemporarilyUnavailableException]
      #
      attr_reader :data
    end
