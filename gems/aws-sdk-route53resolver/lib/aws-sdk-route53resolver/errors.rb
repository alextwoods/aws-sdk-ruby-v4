...
module AWS::SDK::Route53Resolver
  module Errors
    ...
    class InternalServiceErrorException < ApiClientError
      def initialize(http_resp:, **kwargs)
    ...
    class InvalidPolicyDocument < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::InvalidPolicyDocument.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::InvalidPolicyDocument]
      #
      attr_reader :data
    end
