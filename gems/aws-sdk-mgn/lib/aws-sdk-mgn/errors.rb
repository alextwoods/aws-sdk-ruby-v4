...
module AWS::SDK::Mgn
  module Errors
    ...
    class UninitializedAccountException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::UninitializedAccountException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::UninitializedAccountException]
      #
      attr_reader :data
    end
