...
module AWS::SDK::MediaConnect
  module Errors
    ...
    class AddFlowOutputs420Exception < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::AddFlowOutputs420Exception.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::AddFlowOutputs420Exception]
      #
      attr_reader :data
    end
    ...
    class CreateFlow420Exception < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::CreateFlow420Exception.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::CreateFlow420Exception]
      #
      attr_reader :data
    end
    ...
    class GrantFlowEntitlements420Exception < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::GrantFlowEntitlements420Exception.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::GrantFlowEntitlements420Exception]
      #
      attr_reader :data
    end
