...
module AWS::SDK::ApplicationDiscoveryService
  module Errors
    ...
    class ConflictErrorException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ConflictErrorException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ConflictErrorException]
      #
      attr_reader :data
    end

    class HomeRegionNotSetException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::HomeRegionNotSetException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::HomeRegionNotSetException]
      #
      attr_reader :data
    end
    ...
    class ServerInternalErrorException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ServerInternalErrorException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ServerInternalErrorException]
      #
      attr_reader :data
    end
