...
module AWS::SDK::MigrationHubStrategy
  module Errors
    ...
    class ServiceLinkedRoleLockClientException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ServiceLinkedRoleLockClientException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ServiceLinkedRoleLockClientException]
      #
      attr_reader :data
    end
