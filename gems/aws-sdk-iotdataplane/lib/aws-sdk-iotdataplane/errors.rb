...
module AWS::SDK::IoTDataPlane
  module Errors
    ...
    class RequestEntityTooLargeException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::RequestEntityTooLargeException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::RequestEntityTooLargeException]
      #
      attr_reader :data
    end
    ...
    class UnsupportedDocumentEncodingException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::UnsupportedDocumentEncodingException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::UnsupportedDocumentEncodingException]
      #
      attr_reader :data
    end
