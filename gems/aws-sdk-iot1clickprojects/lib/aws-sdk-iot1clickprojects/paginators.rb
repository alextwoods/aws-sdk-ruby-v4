...
module AWS::SDK::IoT1ClickProjects
  module Paginators
    ...
    class ListPlacements
      # @param [Client] client
      # @param [Hash] params (see Client#list_placements)
      # @param [Hash] options (see Client#list_placements)
      def initialize(client, params = {}, options = {})
        @params = params
        @options = options
        @client = client
      end
      # Iterate all response pages of the list_placements operation.
      # @return [Enumerator]
      def pages
        params = @params
        Enumerator.new do |e|
          @prev_token = params[:next_token]
          response = @client.list_placements(params, @options)
          e.yield(response)
      ...
      # Iterate all items from pages in the list_placements operation.
      # @return [Enumerator]
      def items
        Enumerator.new do |e|
          pages.each do |page|
            page.placements.each do |item|
              e.yield(item)
            end
          end
        end
