...
module AWS::SDK::MediaStore
  module Errors
    ...
    class ContainerInUseException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ContainerInUseException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ContainerInUseException]
      #
      attr_reader :data
    end
    ...
    class CorsPolicyNotFoundException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::CorsPolicyNotFoundException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::CorsPolicyNotFoundException]
      #
      attr_reader :data
    end
