...
module AWS::SDK::EKS
  module Errors
    ...
    class ResourcePropagationDelayException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ResourcePropagationDelayException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ResourcePropagationDelayException]
      #
      attr_reader :data
    end
    ...
    class UnsupportedAvailabilityZoneException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::UnsupportedAvailabilityZoneException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::UnsupportedAvailabilityZoneException]
      #
      attr_reader :data
    end
