...
module AWS::SDK::MediaLive
  module Errors
    ...
    class GatewayTimeoutException < ApiServerError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::GatewayTimeoutException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::GatewayTimeoutException]
      #
      attr_reader :data
    end
    ...
    class UnprocessableEntityException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::UnprocessableEntityException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::UnprocessableEntityException]
      #
      attr_reader :data
    end
