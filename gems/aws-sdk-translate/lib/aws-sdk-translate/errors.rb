...
module AWS::SDK::Translate
  module Errors
    ...
    class DetectedLanguageLowConfidenceException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::DetectedLanguageLowConfidenceException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::DetectedLanguageLowConfidenceException]
      #
      attr_reader :data
    end
    ...
    class UnsupportedLanguagePairException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::UnsupportedLanguagePairException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::UnsupportedLanguagePairException]
      #
      attr_reader :data
    end
