...
module AWS::SDK::ComputeOptimizer
  module Errors
    ...
    class MissingAuthenticationToken < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::MissingAuthenticationToken.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::MissingAuthenticationToken]
      #
      attr_reader :data
    end

    class OptInRequiredException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::OptInRequiredException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::OptInRequiredException]
      #
      attr_reader :data
    end
