...
module AWS::SDK::CodeBuild
  module Errors
    ...
    class AccountLimitExceededException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::AccountLimitExceededException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::AccountLimitExceededException]
      #
      attr_reader :data
    end
    ...
    class OAuthProviderException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::OAuthProviderException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::OAuthProviderException]
      #
      attr_reader :data
    end
