...
module AWS::SDK::QLDB
  module Errors
    ...
    class ResourcePreconditionNotMetException < ApiClientError
      def initialize(http_resp:, **kwargs)
        @data = Parsers::ResourcePreconditionNotMetException.parse(http_resp)
        kwargs[:message] = @data.message if @data.respond_to?(:message)

        super(http_resp: http_resp, **kwargs)
      end

      # @return [Types::ResourcePreconditionNotMetException]
      #
      attr_reader :data
    end
